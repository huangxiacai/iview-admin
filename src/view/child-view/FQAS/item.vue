  ...
  export default {
    ...
    methods: {
      ...mapActions([
        ...
        'handleAddFQASListItem',
        'heandleEditFQASListItem',
        'handleDelFQASListItem'
      ]),
      ...
      init() {
        ...
        if(id){
          ...
          this.reqBase.id=id;
          this.handleGetFQASListItem(this.reqBase).then(res => {
            vm.initCallback(res);
          })
        }
       ...
       */
      delOperating(params) {
        this.handleDelFQASListItem(params).then(res => {
          if (res.code === 200) {
        ...
        this.$Modal.confirm({
          ...
          onOk() {
            ...
            getcheckForm.validate(valid => {
              if (valid) {
                //验证后提交数据
                vm.heandleEditFQASListItem(getparams).then(res => {
                  if (res.code === 200) {
        ...
        this.$Modal.confirm({
          ...
          onOk() {
            ...
            getcheckForm.validate(valid => {
              if (valid) {
                //验证后提交数据
                vm.handleAddFQASListItem(getparams).then(res => {
                  if (res.code === 200) {
                    vm.$Message.success(vm.$t('operation_success'));
                    vm.init();
                    vm.$Modal.remove();
